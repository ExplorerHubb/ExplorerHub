...
function Blogs() {
  ...
  const handleLike = async (blogId, liked) => {
    const url = liked
      ? `https://seba2.pythonanywhere.com/Blogs/remove_like/${blogId}/`
      : `https://seba2.pythonanywhere.com/Blogs/add_like/${blogId}/`;
    ...
    try {
      ...
      setBlogs((prevBlogs) =>
        prevBlogs.map((blog) =>
          blog.id === blogId
            ? { ...blog, liked: !liked, likesCount: updatedLikes }
            : blog
        )
      );
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-3 lg:ml-64 m-9 lg:m-0 text-black flex justify-center items-center">
        ...
        <div className="bg-white p-6 shadow-lg rounded-lg h-[84vh] lg:h-[90vh] overflow-y-auto
         flex-1 mx-4 lg:mx-32 custom-scrollbar">
          ...
          <div className="space-y-6">
            {blogs.length > 0 ? (
              blogs.map((blog) => (
                <div
                  key={blog.id}
                  className="border p-5 rounded-lg shadow-lg bg-gray-50"
                >
                    {blog.author === parseInt(user_id) && (
                    ...
                    <div className="mt-4 flex justify-between items-center">
                        ...
                        <button
                            onClick={() => handleLike(blog.id, blog.liked)}
              ...
              <p className="text-red-500">
                No blogs available or unauthorized access.
              </p>
            )}
          </div>
        </div>
